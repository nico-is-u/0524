	...
	<view class="page_buy">
		...
		<view class="pay_box">
			<view style="margin-top: 30rpx;">
				...
				<view v-for="(item, index) in paycd" :key="index">
					<view @click="setSelect(item,index)" :class="['pay_list_item',index==sct?showSelect:'']">
						<image style="width: 1.5rem;height: 1.5rem;margin: 0 20rpx 0 0;transition: all .2s;" v-show="index==sct" :src="item.img" mode="widthFix"></image>
			...
			<u-button iconColor="#fff" class="custom-style"
				text="立即提交" :loading="isDone" :loadingText="regStatus" @click="duBuyBtn"></u-button>
			<!-- 
...
<script>
	export default {
		...
		onLoad() {
			let data = uni.getStorageSync("WHITE_DO_BUY");
			this.dinfo = data;
			let _ = this;
